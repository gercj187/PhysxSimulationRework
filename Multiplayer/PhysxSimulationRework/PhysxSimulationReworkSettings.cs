...
{		
    ...
    public class PhysxSimulationReworkSettings : UnityModManager.ModSettings
    {
		...
		public bool enableStressCouplerBodyDamage = true;
        ...
        public override void Save(UnityModManager.ModEntry modEntry)
		{
			...
			PSR_Multiplayer.NotifySettingsChanged();
        ...
        public void Draw(UnityModManager.ModEntry modEntry)
        {
			...
			if (enableCouplerFailure)
			{
				...
				enableStressCouplerBodyDamage = Toggle(enableStressCouplerBodyDamage,"CouplerBreak do Vehicle Damage");
				...
				GUILayout.Label(
					"<i>(Defines the tensile-force limit at which every eligible coupler joint will break.)</i>",
					italicStyle
				);

				GUILayout.Label(
					"<i>(There is no additional random failure chance.)</i>",
					italicStyle
				);

				GUILayout.Label(
					"<i>(Increasing this value makes couplers more resistant to failure.)</i>",
					italicStyle
				);
				...
				if (Button(
					"Restore Default Break Force (750 kN)",
					Width(500)
				))
				{
