...
namespace PhysxSimulationRework
{
	...
	internal static class TurntableTweaks
	{
		...
		private static readonly HashSet<TurntableController>_pushDetectedLastFrame = new();
		
		private static readonly HashSet<TurntableController>_emergencyStopLocked = new();
		...
		internal static Vector3 GetTurntableWorldPosition(TurntableController tc)
		{
			if (tc == null)
				return Vector3.zero;

			if (tc.turntable != null)
				return tc.turntable.transform.position;

			return tc.transform.position;
		}
		
		private static TurntableController? FindClosestTurntable(Vector3 worldPosition,float maxDistance)
		{
			TurntableController[] controllers =
				UnityEngine.Object.FindObjectsOfType<
					TurntableController>();

			TurntableController? closest = null;
			...
			for (int i = 0; i < controllers.Length; i++)
			{
				...
				Vector3 controllerPosition = GetTurntableWorldPosition(controller);

				float distanceSqr =
					(controllerPosition - worldPosition)
					.sqrMagnitude;
		...
		internal static bool IsSnappingToTarget(TurntableController controller)
		{
			if (controller == null)
				return false;

			TurntableRailTrack turntable =
			...
			float snapAngle =
				_snappingAngle(controller);

			if (snapAngle < 0f)
				return false;

			float currentAngle =
				TurntableRailTrack.AngleRange0To360(
					turntable.currentYRotation
				);
			...
			bool targetReached =
				TurntableRailTrack.AnglesEqual(
					currentAngle,
					snapAngle
				) ||
				TurntableRailTrack.AnglesEqual(
		...
		internal static bool ApplyEmergencyStop(TurntableController controller,string reason,out float stoppedAngle)
		{
			stoppedAngle = 0f;
			...
			TurntableRailTrack turntable = controller.turntable;

			if (turntable == null)
				return false;
			
			if (!IsSnappingToTarget(controller))
			{
				ModLog.Turntable(
					$"Push emergency stop ignored " +
					$"| Reason=Turntable is not snapping " +
					$"| Source={reason}"
				);
			...
			turntable.targetYRotation =	stoppedAngle;

			turntable.RotateToTargetRotation();

			_snappingAngle(controller) = -1f;
			_snappingAngleSet(controller) = true;
			_snappingDirection(controller) = 0f;

			_rotationSoundIntensity(controller) = 0f;
			_playTrackConnectedSound(controller) = false;
			_playSnapRangeEnterSound(controller) = false;
			_lastSnappingAnglePlayed(controller) = -1f;

			_snapBrakeClamp.Remove(controller);
			_nextBellTime.Remove(controller);
			_lastDriveDir.Remove(controller);
			_lastWasPush.Remove(controller);

			_pushingPositiveDirectionValue(controller) = 0f;
			_pushingNegativeDirectionValue(controller) = 0f;

			_emergencyStopLocked.Add(controller);

			ModLog.Turntable(
				$"SNAPPING EMERGENCY STOP applied " +
				$"| Reason={reason} " +
		...
		internal static bool ApplyEmergencyStop(TurntableController controller,string reason)
		{
			float ignoredStoppedAngle;

			return ApplyEmergencyStop(
				controller,
				reason,
				out ignoredStoppedAngle
			);
		}
		
		internal static bool ApplyEmergencyStopFromNetwork(Vector3 worldPosition,out float stoppedAngle)
		{
			stoppedAngle = 0f;

			const float maxMatchDistance = 25f;

			TurntableController? controller =
				FindClosestTurntable(
					worldPosition,
					maxMatchDistance
				);
			...
			return ApplyEmergencyStop(
				controller,
				"Client push received by host",
				out stoppedAngle
			);
		}
		
		internal static bool ApplyEmergencyStopFromNetwork(Vector3 worldPosition,float authoritativeStoppedAngle)
		{
			const float maxMatchDistance = 25f;
			...
			ModLog.Turntable(
				$"AUTHORITATIVE TURNTABLE STOP applied " +
				$"| Angle={stoppedAngle:F2}"
			);
		...
		internal static void PlayBellFromNetwork(Vector3 networkPosition)
		{
			TurntableController[] controllers =
				UnityEngine.Object.FindObjectsOfType<
					TurntableController>();
			...
			for (int i = 0; i < controllers.Length; i++)
			{
				...
				Vector3 localPosition =
					GetTurntableWorldPosition(controller);

				float distanceSqr =
					(localPosition - networkPosition)
					.sqrMagnitude;
			...
			if (closest == null || closestDistanceSqr > maxMatchDistance * maxMatchDistance)
			{
				ModLog.Turntable(
					$"No local turntable found for network sound " +
					$"| Position={networkPosition} " +
					$"| BestDistance=" +
					$"{Mathf.Sqrt(closestDistanceSqr):F1} m"
				);

				return;
			}

			PlayBellLocal(closest);

			ModLog.Turntable(
				$"Network turntable matched " +
				$"| Position={networkPosition} " +
				$"| Distance=" +
				$"{Mathf.Sqrt(closestDistanceSqr):F1} m"
			);
		...
		internal static void PlayBellLocal(TurntableController tc)
		{
		...
		private static void HandleBell(TurntableController tc)
		{
			...
			if (Time.time >= next)
			{
				PSR_Multiplayer.NotifyLocalTurntableBell(
					tc
				);
		...
		[HarmonyTargetMethod]
		private static MethodBase TargetMethod() => AccessTools.Method(typeof(TurntableController), "FixedUpdate");
		...
		private static bool FixedUpdate_Prefix(TurntableController __instance)
		{
			...
			float value =
				(__instance.PlayerControlAllowed
					? lever.Value
					: 0.5f);
			...
			if (pushDetected)
			{
				if (IsSnappingToTarget(__instance))
				{
					if (_pushDetectedLastFrame.Add(__instance))
					{
						PSR_Multiplayer.NotifyLocalTurntablePushStop(
							__instance
						);
					}
					_rotationSoundIntensity(__instance) = 0f;
			...
			{
				_pushDetectedLastFrame.Remove(__instance);
			}

			if (_emergencyStopLocked.Contains(__instance))
			{
				float positiveLeverInput =
					Mathf.InverseLerp(
						0.55f,
						1f,
						value
				...
				bool leverNeutral =
					positiveLeverInput <= 0f &&
					negativeLeverInput <= 0f;

				if (!pushDetected && leverNeutral)
				{
					_emergencyStopLocked.Remove(__instance);

					ModLog.Turntable(
						"Turntable emergency-stop lock released"
					);
				}
				else
				{
					turntable.targetYRotation =
						TurntableRailTrack.AngleRange0To360(
							turntable.currentYRotation
						);
			...
			float posInput = Mathf.InverseLerp(0.55f,1f,value);
			if (posInput > 0f)
			{
			...
			float negInput = Mathf.InverseLerp(0.45f,0f,value);
			if (negInput > 0f)
			{
				_lastDriveDir[__instance] = -1;
	...
	public static class TrainCar_Derail_Patch
	{
		static void Postfix(TrainCar __instance)
		{
			...
			if (!PSR_Multiplayer.CanRunAuthoritativeGameplay)
				return;
	...
	public static class Coupler_CreateRigidJoint_Patch
	{
		...
		static void Postfix(Coupler __instance)
		{
					...
					.IsNetworkBroken(__instance))
			{
				...
				if (rigidJointField != null)
				{
					rigidJointField.SetValue(
						__instance,
						null
					);
				}
				...
				ModLog.Coupler(
					"Blocked recreation of synchronized " +
					"broken client coupler joint"
				);
			...
			if (!WeakCouplerRegistry.IsMarked(
				__instance))
			{
				return;
			}
			...
			ModLog.Derail(
				"[PhysxSimulationRework] " +
				"breakForce overridden AFTER CreateJoint"
			);
		}
	}
	...
	{
		private static readonly HashSet<int> markedJointIds = new HashSet<int>();
	...
	// =====================================================
	// CLIENT COUPLER BREAK APPLICATION
	// =====================================================

	internal static class CouplerBreakSynchronizer
{
	private static readonly FieldInfo?
		rigidJointField =
			AccessTools.Field(
	...
	// Dauerhafte lokale Sperre gegen erneute Joint-Erzeugung.
	...
	internal static bool IsNetworkBroken(
		Coupler coupler)
	{
		if (coupler == null)
			return false;
		...
		if (car == null ||
			string.IsNullOrEmpty(car.CarGUID))
		{
			return false;
		}
		...
		return brokenCouplerKeys.Contains(
			GetKey(
				car.CarGUID,
				isFront
			)
	...
	internal static bool ApplyNetworkCouplerBreak(
		string carGuidA,
		bool isFrontA,
		string carGuidB,
		bool isFrontB)
	{
		...
		TrainCarRegistry? registry =
			SingletonBehaviour<
				TrainCarRegistry>.Instance;

		if (registry == null)
			return false;

		TrainCar? carA =
			registry.GetTrainCarByCarGuid(
				carGuidA
			);
		...
		// Beide Enden dauerhaft als gebrochen merken.
		brokenCouplerKeys.Add(
			GetKey(
				carGuidA,
				isFrontA
			)
		...
		// Alle gefundenen lokalen Joints sammeln.
		HashSet<ConfigurableJoint> joints =
		...
		// Referenzen auf beiden eindeutigen Enden leeren.
		if (rigidJointField != null)
		{
			rigidJointField.SetValue(
				couplerA,
				...
				in joints)
		{
			...
			Coupler[] allCouplers =
				UnityEngine.Object
					.FindObjectsOfType<Coupler>();

			for (int i = 0;
				i < allCouplers.Length;
				i++)
			{
				Coupler localCoupler =
					allCouplers[i];

				if (localCoupler == null)
					continue;

				if (!ReferenceEquals(
					localCoupler.rigidCJ,
					joint))
				{
					continue;
				}
		...
		ModLog.Coupler(
			$"Applied authoritative coupler break " +
			$"| A={carA.ID}/" +
			$"{(isFrontA ? "FRONT" : "REAR")} " +
			$"| B={carB.ID}/" +
			$"{(isFrontB ? "FRONT" : "REAR")} " +
			$"| DestroyedJoints={joints.Count}"
		);
    ...
    internal class CouplerJointBreakListener : MonoBehaviour
	{
		private void OnJointBreak(float breakForce)
		{
			...
			// -------------------------------------------------
			// 5 % BODY DAMAGE NUR BEI EINEM STRESSBRUCH
			// -------------------------------------------------
			...
			if (!causedByDerailment &&
				...
				settings.enableStressCouplerBodyDamage)
			{
				ApplyStressBreakBodyDamage(
					couplers,
					jointId
				);
			}
			else if (causedByDerailment)
			{
				ModLog.Coupler(
			...
			Coupler? couplerA = null;
			Coupler? couplerB = null;
				...
				i++)
			{
				...
				TrainCar? candidateCar =
					candidate.GetComponentInParent<TrainCar>();

				if (candidateCar == null ||
					string.IsNullOrEmpty(candidateCar.CarGUID))
				{
					continue;
				}
				...
				// Das zweite Ende muss zu einem anderen Wagen gehören.
				if (!ReferenceEquals(
					carA,
					candidateCar))
				{
			...
			{
				ModLog.Coupler(
					$"Broken joint could not be mapped to " +
					$"two distinct couplers " +
					$"| JointId={jointId} " +
					$"| RegisteredCount={couplers.Count}"
				);

				return;
			}

			PSR_Multiplayer.HostBroadcastCouplerBreak(
				couplerA,
				couplerB
			);

			ModLog.Coupler(
				$"Broken joint synchronization requested " +
				$"| JointId={jointId} " +
				$"| BreakForce={breakForce:F1}"
			);
		}
		...
		private static void ApplyStressBreakBodyDamage(List<Coupler> couplers,int jointId)
		{
			...
			try
			{
				...
				ModLog.Coupler(
					$"Coupler-break body damage applied by host " +
					$"| Joint={jointId} " +
					$"| Car={affectedCar.ID} " +
				...
				PSR_Multiplayer.HostBroadcastDamageState(
					affectedCar,
					"Stress-related coupler failure"
				);
			}
			catch (Exception ex)
			{
		...
		static void Postfix(Bogie __instance)
		{
			...
			if (riskLevel[car] >= settings.riskThreshold)
			{
				ModLog.Risk(
					$"DERAIL TRIGGERED " +
					$"| Car={carId} ({carType}) " +
					$"| Risk={riskLevel[car]:F2} " +
					$"| Speed={speedKmh:F0}"
				);

				riskLevel[car] = 0f;

				PSR_Multiplayer.HostDerailCar(
					car,
					"Dynamic derail system"
				);
			}
		}
	}
